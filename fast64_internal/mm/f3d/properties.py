...
from ...f3d.f3d_parser import mmEnumDrawLayers
from ...utility import prop_split


class MMDLExportSettings(PropertyGroup):
    ...
    drawLayer: EnumProperty(name="Draw Layer", items=mmEnumDrawLayers)
...
class MMDLImportSettings(PropertyGroup):
...
class MMDynamicMaterialDrawLayerProperty(PropertyGroup):
...
class MMDynamicMaterialProperty(PropertyGroup):
    opaque: PointerProperty(type=MMDynamicMaterialDrawLayerProperty)
    transparent: PointerProperty(type=MMDynamicMaterialDrawLayerProperty)
    ...
    def draw_props(self, layout: UILayout, mat: Object, drawLayer: str):
        ...
        layout.box().column().label(text="MM Dynamic Material Properties " + suffix)
        layout.label(text="See gSPSegment calls in z_scene_table.c.")
        layout.label(text="Based off draw config index in gSceneTable.")
        dynMatLayerProp: MMDynamicMaterialDrawLayerProperty = getattr(self, drawLayer.lower())
...
class MMDefaultRenderModesProperty(PropertyGroup):
...
mm_dl_writer_classes = (
    MMDLExportSettings,
    MMDLImportSettings,
    MMDynamicMaterialDrawLayerProperty,
    MMDynamicMaterialProperty,
    MMDefaultRenderModesProperty,
)


def mm_f3d_props_register():
    mmEnumObjectMenu = [
    ...
    Object.mmDrawLayer = EnumProperty(items=mmEnumDrawLayers, default="Opaque")

    # Doesn't work since all static meshes are pre-transformed
    # Object.mmDynamicTransform = PointerProperty(type = MMDynamicTransformProperty)
    World.mmDefaultRenderModes = PointerProperty(type=MMDefaultRenderModesProperty)
    Material.mmMaterial = PointerProperty(type=MMDynamicMaterialProperty)
    Object.mmObjectMenu = EnumProperty(items=mmEnumObjectMenu)


def mm_f3d_props_unregister():
    for cls in reversed(mm_dl_writer_classes):
        unregister_class(cls)

    del Material.mmMaterial
    del Object.mmObjectMenu
