...
from ...f3d.f3d_material import mmEnumDrawLayers
from ...utility import prop_split
from .constants import mmEnumSkeletonImportMode
...
class MMDynamicTransformProperty(PropertyGroup):
...
class MMBoneProperty(PropertyGroup):
    boneType: EnumProperty(name="Bone Type", items=mmEnumBoneType)
    dynamicTransform: PointerProperty(type=MMDynamicTransformProperty)
...
class MMSkeletonProperty(PropertyGroup):
...
class MMSkeletonExportSettings(PropertyGroup):
    ...
    mode: EnumProperty(name="Mode", items=mmEnumSkeletonImportMode)
...
class MMSkeletonImportSettings(PropertyGroup):
    mode: EnumProperty(name="Mode", items=mmEnumSkeletonImportMode)
    ...
    drawLayer: EnumProperty(name="Import Draw Layer", items=mmEnumDrawLayers)
    actorOverlayName: StringProperty(name="Overlay", default="ovl_En_GeldB")
    flipbookUses2DArray: BoolProperty(name="Has 2D Flipbook Array", default=False)
    flipbookArrayIndex2D: IntProperty(name="Index if 2D Array", default=0, min=0)
...
mm_skeleton_classes = (
    MMDynamicTransformProperty,
    MMBoneProperty,
    MMSkeletonProperty,
    MMSkeletonExportSettings,
    MMSkeletonImportSettings,
)


def mm_skeleton_props_register():
    for cls in mm_skeleton_classes:
        register_class(cls)

    Object.mmActorScale = FloatProperty(min=0, default=100)
    Object.mmSkeleton = PointerProperty(type=MMSkeletonProperty)
    Bone.mmBone = PointerProperty(type=MMBoneProperty)


def mm_skeleton_props_unregister():
    del Object.mmActorScale
    del Bone.mmBone
    del Object.mmSkeleton
