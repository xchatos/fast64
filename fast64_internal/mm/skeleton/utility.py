...
from ..mm_model_classes import MMVertexGroupInfo
from ..mm_utility import checkForStartBone, getStartBone, getNextBone
...
def mmGetSkeleton(skeletonData, skeletonName, continueOnError):
...
def mmGetLimbs(skeletonData, limbsName, continueOnError):
    matchResult = re.search(
...
def mmGetLimb(skeletonData, limbName, continueOnError):
...
def getGroupIndices(meshInfo, armatureObj, meshObj,rootGroupIndex):
    meshInfo.vertexGroupInfo = MMVertexGroupInfo()
...
def mmRemoveSkeleton(filepath, objectName, skeletonName):
    ...
    matchResult = mmGetSkeleton(skeletonDataC, skeletonName, True)
    if matchResult is None:
        return
    ...
    matchResult = mmGetLimbs(skeletonDataC, limbsName, True)
    if matchResult is None:
        return
    skeletonDataC = skeletonDataC[: matchResult.start(0)] + skeletonDataC[matchResult.end(0) :]
    ...
    for limb in limbList:
        matchResult = mmGetLimb(skeletonDataC, limb, True)
...
def mmRemoveRotationsFromBone(armatureObj: bpy.types.Object, bone: bpy.types.Bone):
    for childBone in bone.children:
        mmRemoveRotationsFromBone(armatureObj, childBone)
...
def mmRemoveRotationsFromArmature(armatureObj: bpy.types.Object) -> None:
    ...
    mmRemoveRotationsFromBone(armatureObj, armatureObj.data.bones[startBoneName])
    armatureApplyWithMesh(armatureObj, bpy.context)


def mmDuplicateArmatureAndRemoveRotations(originalArmatureObj: bpy.types.Object):
    ...
    try:
        ...
        mmRemoveRotationsFromArmature(armatureObj)
