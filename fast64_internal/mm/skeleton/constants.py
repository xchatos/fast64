...
# 3. Select skeleton, then run bpy.ops.object.mm_save_rest_pose()
# 4. Copy array data from console into an MMSkeletonImportInfo object
#       - list of tuples, first is root position, rest are euler XYZ rotations
# 5. Add object to mmSkeletonImportDict


# Link overlay will be "", since Link texture array data is handled as a special case.
class MMSkeletonImportInfo:
    def __init__(
        self,
        skeletonName: str,
        folderName: str,
...
mmSkeletonImportDict = OrderedDict(
    {
        "Adult Link": MMSkeletonImportInfo(
            "gLinkAdultSkel",
            "object_link_boy",
            "",
            0,
        ...
        "Child Link": MMSkeletonImportInfo(
            "gLinkChildSkel",
            "object_link_child",
            "",
            1,
        ...
        # "Gerudo": MMSkeletonImportInfo("gGerudoRedSkel", "object_geldb", "ovl_En_GeldB", None, None),
    }
)

mmEnumSkeletonImportMode = [
    ("Generic", "Generic", "Generic"),
]

for name, info in mmSkeletonImportDict.items():
    mmEnumSkeletonImportMode.append((name, name, name))
