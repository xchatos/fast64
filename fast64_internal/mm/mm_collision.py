    ...
    for polygonType, faces in collisionDict.items():
        ...
        for faceVerts, normal, distance in faces:
            ...
            for roundedPosition in faceVerts:
                ...
                if index is None:
                    collision.vertices.append(MMCollisionVertex(roundedPosition))
            ...
            # https://github.com/zeldaret/mm/blob/791d9018c09925138b9f830f7ae8142119905c05/src/code/z_bgcheck.c#L161
            #
            ...
            # https://github.com/zeldaret/mm/blob/791d9018c09925138b9f830f7ae8142119905c05/src/code/z_bgcheck.c#L2888
            ...
            collision.polygonGroups[polygonType].append(MMCollisionPolygon(indices, normal, distance))
...
def addCollisionTriangles(obj, collisionDict, includeChildren, transformMatrix, bounds):
    if isinstance(obj.data, bpy.types.Mesh) and not obj.ignore_collision:
        ...
        for face in obj.data.loop_triangles:
            ...
            polygonType = getPolygonType(material.mmCollisionProperty)
...
def mmCollisionVertexToC(vertex):
    return "{ " + str(vertex.position[0]) + ", " + str(vertex.position[1]) + ", " + str(vertex.position[2]) + " },\n"


def mmCollisionPolygonToC(polygon, ignoreCamera, ignoreActor, ignoreProjectile, enableConveyor, polygonTypeIndex):
    return (
        "{ "
        + ", ".join(
            (
...
def mmCameraDataToC(camData):
    ...
    if len(camData.camPosDict) > 0:
        ...
        for i in range(len(camData.camPosDict)):
            ...
            if isinstance(camItem, MMCameraPosData):
                camC.source += "\t" + mmCameraEntryToC(camItem, camData, camPosIndex) + ",\n"
                if camItem.hasPositionData:
                    posC.source += mmCameraPosToC(camItem)
                    camPosIndex += 3
            elif isinstance(camItem, MMCrawlspaceData):
                camC.source += "\t" + mmCrawlspaceEntryToC(camItem, camData, camPosIndex) + ",\n"
                posC.source += mmCrawlspaceToC(camItem)
...
def mmCameraEntryToC(camPos, camData, camPosIndex):
    return " ".join(
        (
            "{",
            camPos.camSType + ",",
...
def mmCrawlspaceToC(camItem: MMCrawlspaceData):
    data = ""
...
def mmCrawlspaceEntryToC(camItem: MMCrawlspaceData, camData: MMCameraData, camPosIndex: int):
    return " ".join(
        (
            "{",
            camItem.camSType + ",",
...
def mmCollisionToC(collision):
    data = CData()
    posC, camC = mmCameraDataToC(collision.cameraData)
    ...
    if len(collision.polygonGroups) > 0:
        ...
        for polygonType, polygons in collision.polygonGroups.items():
            polygonTypeC += "\t" + mmPolygonTypeToC(polygonType)
            for polygon in polygons:
                polygonC += "\t" + mmCollisionPolygonToC(
                    polygon,
    ...
    if len(collision.vertices) > 0:
        ...
        for vertex in collision.vertices:
            data.source += "\t" + mmCollisionVertexToC(vertex)
        data.source += "};\n\n"
    ...
    if len(collision.waterBoxes) > 0:
        ...
        for waterBox in collision.waterBoxes:
            data.source += "\t" + mmWaterBoxToC(waterBox)
        data.source += "};\n\n"
