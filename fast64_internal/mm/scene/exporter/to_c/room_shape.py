...
from ....mm_constants import mmRoomShapeStructs, mmRoomShapeEntryStructs, mmEnumRoomShapeType
from ....mm_level_classes import MMRoom, MMRoomMeshGroup, MMRoomMesh


def getRoomShapeDLEntry(meshEntry: MMRoomMeshGroup, roomShape: str):
...
def getRoomShapeImageData(roomMesh: MMRoomMesh, textureSettings: TextureExportSettings):
    code = CData()
...
def getRoomShape(outRoom: MMRoom):
    roomShapeInfo = CData()
    roomShapeDLArray = CData()
    mesh = outRoom.mesh

    shapeTypeIdx = [value[0] for value in mmEnumRoomShapeType].index(mesh.roomShape)
    dlEntryType = mmRoomShapeEntryStructs[shapeTypeIdx]
    structName = mmRoomShapeStructs[shapeTypeIdx]
...
def getRoomModel(outRoom: MMRoom, textureExportSettings: TextureExportSettings):
    roomModel = CData()
    mesh = outRoom.mesh
    ...
    roomModel.append(mesh.model.to_c(textureExportSettings, MMGfxFormatter(ScrollMethod.Vertex)).all())
