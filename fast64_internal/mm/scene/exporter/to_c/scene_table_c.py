...
from ....mm_constants import mmEnumSceneID, mmSceneNameToID
from ....mm_utility import getCustomProperty, ExportInfo


def getSceneNameSettings(scene):
    if scene is not None:
        return bpy.context.scene.mmSceneExportSettings.option
    else:
        return bpy.context.scene.mmSceneRemoveSettings.option


def getHackerMmCheck(line: str):
    return (
        line != "\n"
    ...
    try:
        with open(os.path.join(exportPath, "include/tables/scene_table.h")) as fileData:
            ...
            for i, line in enumerate(fileData):
                ...
                if not bpy.context.scene.fast64.mm.hackerFeaturesEnabled or getHackerMmCheck(line):
                    if not (
...
def getInsertionIndex(scene, sceneNames, sceneName, index, mode):
    ...
    for i in range(len(sceneNames)):
        if sceneNames[i] == mmEnumSceneID[currentIndex][0]:
...
def getSceneParams(scene, exportInfo, sceneNames):
    ...
    if sceneIndex is None and scene is not None:
        ...
        sceneTitle = "none"
        sceneID = mmSceneNameToID.get(name, f"SCENE_{name.upper()}")
...
def getDrawConfig(sceneName: str):
    ...
    fileData, header, sceneNames = getSceneTable(bpy.path.abspath(bpy.context.scene.mmDecompPath))
...
def addHackerMmData(fileData: str):
    """Reads the file and adds HackerMm's modifications to the scene table file"""
...
def modifySceneTable(scene, exportInfo: ExportInfo):
    ...
    # sceneIndex can be None and mmSceneOption not "Custom",
    ...
    # apply HackerMm changes if needed
    if bpy.context.scene.fast64.mm.hackerFeaturesEnabled:
        newFileData = addHackerMmData(newFileData)
