...
from .mm_collision_classes import MMCollision
...
class MMSceneTableEntry:
    def __init__(self):
        self.drawConfig = 0


class MMScene(MMCommonCommands):
    def __init__(self, name, model):
        ...
        self.lights = []
        self.model = model
        self.collision = MMCollision(self.name)
        ...
        self.childNightHeader: Optional[MMScene] = None
        self.adultDayHeader: Optional[MMScene] = None
        self.adultNightHeader: Optional[MMScene] = None
        self.cutsceneHeaders: list["MMScene"] = []
        ...
        self.sceneTableEntry = MMSceneTableEntry()

    def getAlternateHeaderScene(self, name):
        scene = MMScene(name, self.model)
    ...
    def addRoom(self, roomIndex, roomName, roomShape):
        roomModel = self.model.addSubModel(MMModel(roomName + "_dl", self.model.DLFormat, None))
        room = MMRoom(roomIndex, roomName, roomModel, roomShape)
    ...
    def sortEntrances(self):
        ...
        if self.appendNullEntrance:
            self.entranceList.append(MMEntrance(0, 0))
...
class MMRoomMesh:
    ...
    def addMeshGroup(self, cullGroup):
        meshGroup = MMRoomMeshGroup(cullGroup, self.model.DLFormat, self.roomName, len(self.meshEntries))
...
class MMRoom(MMCommonCommands):
    ...
    def getAlternateHeaderRoom(self, name):
        room = MMRoom(self.index, name, self.mesh.model, self.mesh.roomShape)
        room.mesh = self.mesh
        return room
