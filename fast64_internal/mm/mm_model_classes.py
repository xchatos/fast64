...
from ..f3d.flipbook import TextureFlipbook, FlipbookProperty, usesFlipbook, mmFlipbookReferenceIsValid
...
def mmGetIncludedAssetData(basePath: str, currentPaths: list[str], data: str) -> str:
    includeData = ""
...
def mmGetActorDataPaths(basePath: str, overlayName: str) -> list[str]:
...
# read actor data
def mmGetActorData(basePath: str, overlayName: str) -> str:
    actorData = getImportData(mmGetActorDataPaths(basePath, overlayName))
    return actorData


def mmGetLinkData(basePath: str) -> str:
...
class MMModel(FModel):
    ...
    def getRenderMode(self, drawLayer):
        ...
        defaultRenderModes = bpy.context.scene.world.mmDefaultRenderModes
    ...
    def processTexRefCITextures(self, fMaterial: FMaterial, material: bpy.types.Material, index: int) -> FImage:
        ...
        flipbookProp = getattr(material.flipbookGroup, f"flipbook{index}")
        texProp = getattr(material.f3d_mat, f"tex{index}")
        if not usesFlipbook(material, flipbookProp, index, True, mmFlipbookReferenceIsValid):
    ...
    def onMaterialCommandsBuilt(self, fMaterial, material, drawLayer):
        ...
        matDrawLayer = getattr(material.mmMaterial, drawLayer.lower())
        for i in range(8, 14):
    ...
    def onAddMesh(self, fMesh, contextObj):
        if contextObj is not None and hasattr(contextObj, "mmDynamicTransform"):
            if contextObj.mmDynamicTransform.billboard:
                fMesh.draw.commands.append(SPMatrix("0x01000000", "G_MTX_MODELVIEW | G_MTX_NOPUSH | G_MTX_MUL"))


class MMDynamicMaterialDrawLayer:
    def __init__(self, opaque, transparent):
        self.opaque = opaque
        self.transparent = transparent


class MMGfxFormatter(GfxFormatter):
    def __init__(self, scrollMethod):
        GfxFormatter.__init__(self, scrollMethod, 64, None)


class MMTriangleConverterInfo(TriangleConverterInfo):
...
class MMVertexGroupInfo(VertexGroupInfo):
    def __init__(self):
...
class MMF3DContext(F3DContext):
    ...
    def processDLName(self, name):
        ...
        try:
            ...
            if segment >= 0x08 and segment <= 0x0D:
                setattr(self.materialContext.mmMaterial.opaque, "segment" + format(segment, "1X"), True)
                setattr(self.materialContext.mmMaterial.transparent, "segment" + format(segment, "1X"), True)
    ...
    def processTextureName(self, textureName):
        try:
            ...
            return textureName
            # if (pointer >> 24) == 0x08:
            # 	print("Unhandled MM pointer: " + textureName)

    def getMaterialKey(self, material: bpy.types.Material):
        return (material.mmMaterial.key(), material.f3d_mat.key())
    ...
    def clearMaterial(self):
        ...
        # Don't clear mmMaterial, some skeletons (Link) require dynamic material calls to be preserved between limbs
        clearMMFlipbookProperty(self.materialContext.flipbookGroup.flipbook0)
        clearMMFlipbookProperty(self.materialContext.flipbookGroup.flipbook1)
    ...
    def handleTextureReference(
        ...
        clearMMFlipbookProperty(getattr(material.flipbookGroup, "flipbook" + str(index)))
        match = re.search(f"(0x0[0-9a-fA-F])000000", name)
        if match:
            segment = int(match.group(1), 16)
            flipbookKey = (segment, material.f3d_mat.draw_layer.mm)
                ...
                for textureName in flipbook.textureNames:
                    ...
                    if not isinstance(image, bpy.types.Image):
                        raise PluginError(
                            ...
                            f"For MM scenes this may be because the scene's draw config references textures not stored in its scene/room files.\n"
    ...
    def handleApplyTLUT(
        ...
        if usesFlipbook(material, flipbook, index, True, mmFlipbookReferenceIsValid):
...
def clearMMFlipbookProperty(flipbookProp):
...
def clearMMMaterialDrawLayerProperty(matDrawLayerProp):
