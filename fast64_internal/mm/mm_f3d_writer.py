...
from .mm_utility import replaceMatchContent, getMMScale
from .mm_texture_array import TextureFlipbook
...
from .mm_model_classes import (
    MMTriangleConverterInfo,
    MMModel,
    mmGetActorData,
    mmGetLinkData,
)
...
def mmProcessVertexGroup(
    ...
    if len(groupFaces) == 0:
        ...
        # MM will only allocate matrix if DL exists.
    ...
    meshInfo.vertexGroupInfo.vertexGroupToMatrixIndex[currentGroupIndex] = nextDLIndex
    triConverterInfo = MMTriangleConverterInfo(meshObj, armatureObj.data, fModel.f3d, convertTransformMatrix, meshInfo)
    ...
    # however it seems like MM skeletons don't have this ability.
...
def writeTextureArraysNew(fModel: MMModel, arrayIndex: int):
...
def writeTextureArraysExisting(
    exportPath: str, overlayName: str, isLink: bool, flipbookArrayIndex2D: int, fModel: MMModel
):
...
def mmReadActorScale(basePath: str, overlayName: str, isLink: bool) -> float:
    if not isLink:
        actorData = mmGetActorData(basePath, overlayName)
    else:
        actorData = mmGetLinkData(basePath)
    ...
    if chainInitMatch is not None:
        ...
        if scale[-1] == "f":
            scale = scale[:-1]
        return getMMScale(1 / (float(scale) / 1000))
    ...
    if actorScaleMatch is not None:
        ...
        if scale[-1] == "f":
            scale = scale[:-1]
        return getMMScale(1 / float(scale))

    return getMMScale(100)
