...
def mmReadTextureArrays(
    ...
    if not isLink:
        ...
        currentPaths = mmGetActorDataPaths(basePath, overlayName)
    else:
        actorData = mmGetLinkData(basePath)
        currentPaths = [os.path.join(basePath, f"src/code/z_player_lib.c")]
    actorData = mmGetIncludedAssetData(basePath, currentPaths, actorData) + actorData
    ...
    if not isLink and overlayName == "ovl_En_Ossan":
        ...
        mmReadTextureArraysFromMultiple(
        ...
        mmReadTextureArraysGeneric(flipbookList, actorData, getSPSegmentCalls, f3dContext)


# we return when no matches found to handle cases where actor does not have dynamic textures.
def mmReadTextureArraysFromMultiple(
    ...
    mmReadTextureArraysGeneric(flipbookList, drawData, getSegmentCallsFunc, f3dContext)


def mmReadTextureArraysGeneric(
    flipbookList: dict[str, TextureFlipbook],
    actorData: str,
    getSegmentCallsFunc: Callable[[str], None],
    f3dContext: MMF3DContext,
