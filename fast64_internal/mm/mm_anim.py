...
from .skeleton.exporter import mmConvertArmatureToSkeletonWithoutMesh
from .mm_model_classes import mmGetIncludedAssetData
...
def mmGetAnimBoneRot(bone, poseBone, convertTransformMatrix, isRoot):
    # Mm draws limbs like this:
    ...
    # The translation is computed in mmProcessBone as
    ...
    # This should be very close to only a rotation, or if rmm, only a rotation
    # and translation.
    finalScale = [finalScale.x, finalScale.y, finalScale.z]
    if max(finalScale) >= 1.01 or min(finalScale) <= 0.99:
        raise RuntimeError("Animation contains bones with animated scale. Mm SkelAnime does not support this.")
    ...
    if not isRoot and (max(finalTranslation) >= 1.0 or min(finalTranslation) <= -1.0):
        raise RuntimeError(
            "Animation contains non-root bones with animated translation. Mm SkelAnime only supports animated translation on the root bone."
        )
    return finalRotation


def mmConvertNonLinkAnimationData(anim, armatureObj, convertTransformMatrix, *, frame_start, frame_count):
    ...
    # must be SAME order as mmProcessBone
    ...
    # since we are processing the bones in the same order as mmProcessBone,
    ...
    for frame in range(frame_start, frame_start + frame_count):
        ...
        # Convert Z-up to Y-up for rmm translation animation
        translation = (
        ...
        for boneIndex in range(len(animBones)):
            ...
            saveQuaternionFrame(
                ...
                mmGetAnimBoneRot(currentBone, currentPoseBone, convertTransformMatrix, boneIndex == 0),
            )
...
def mmConvertLinkAnimationData(anim, armatureObj, convertTransformMatrix, *, frame_start, frame_count):
    checkForStartBone(armatureObj)
    bonesToProcess = [getStartBone(armatureObj)]
    currentBone = armatureObj.data.bones[bonesToProcess[0]]
    animBones = []
    ...
    for frame in range(frame_start, frame_start + frame_count):
        ...
        for boneIndex in range(len(animBones)):
            ...
            rotation = mmGetAnimBoneRot(currentBone, currentPoseBone, convertTransformMatrix, boneIndex == 0)
            for i in range(3):
                field = rotation.to_euler()[i]
                value = (math.degrees(field) % 360) / 360
                frameData.append(min(int(round(value * (2**16 - 1))), 2**16 - 1))

        textureAnimValue = (armatureObj.mmLinkTextureAnim.eyes & 0xF) | (
            (armatureObj.mmLinkTextureAnim.mouth & 0xF) << 4
        )
...
def mmExportNonLinkAnimation(armatureObj, convertTransformMatrix, skeletonName):
    ...
    mmAnim = MMAnimation(toAlnum(skeletonName + anim.name.capitalize() + "Anim"))

    skeleton = mmConvertArmatureToSkeletonWithoutMesh(armatureObj, convertTransformMatrix, skeletonName)

    frame_start, frame_last = getFrameInterval(anim)
    mmAnim.frameCount = frame_last - frame_start + 1

    armatureFrameData = mmConvertNonLinkAnimationData(
        anim,
        armatureObj,
    ...
    for frameData in singleFrameData:
        ...
        if frameData.boneIndex not in mmAnim.indices:
            mmAnim.indices[frameData.boneIndex] = [None, None, None]
        if frame in mmAnim.values:
            mmAnim.indices[frameData.boneIndex][frameData.field] = mmAnim.values.index(frame)
        else:
            mmAnim.indices[frameData.boneIndex][frameData.field] = len(mmAnim.values)
            mmAnim.values.extend(frameData.frames)

    mmAnim.limit = len(mmAnim.values)
...
def mmExportLinkAnimation(armatureObj, convertTransformMatrix, skeletonName):
    if armatureObj.animation_data is None or armatureObj.animation_data.action is None:
        raise PluginError("No active animation selected.")
    ...
    mmAnim = MMLinkAnimation(toAlnum(skeletonName + anim.name.capitalize() + "Anim"))

    frame_start, frame_last = getFrameInterval(anim)
    mmAnim.frameCount = frame_last - frame_start + 1

    mmAnim.data = mmConvertLinkAnimationData(
        anim,
        armatureObj,
        convertTransformMatrix,
        frame_start=frame_start,
...
def mmImportNonLinkAnimationC(armatureObj, filepath, animName, actorScale, isCustomImport: bool):
    animData = getImportData([filepath])
    if not isCustomImport:
        basePath = bpy.path.abspath(bpy.context.scene.mmDecompPath)
        animData = mmGetIncludedAssetData(basePath, [filepath], animData) + animData
            ...
            for frame in range(frameCount):
                ...
                for propertyIndex in range(3):
                    if jointIndex[propertyIndex] < staticIndexMax:
                        value = mmTranslationValue(frameData[jointIndex[propertyIndex]], actorScale)
                    else:
                        value = mmTranslationValue(frameData[jointIndex[propertyIndex] + frame], actorScale)
...
def mmImportLinkAnimationC(
    ...
    if not isCustomImport:
        basePath = bpy.path.abspath(bpy.context.scene.mmDecompPath)
        animHeaderData = mmGetIncludedAssetData(basePath, [animHeaderFilepath], animHeaderData) + animHeaderData
        animData = mmGetIncludedAssetData(basePath, [animFilepath], animData) + animData
    ...
    eyesCurve = anim.fcurves.new(
        data_path="mmLinkTextureAnim.eyes",
        action_group="Texture Animations",
    )
    mouthCurve = anim.fcurves.new(
        data_path="mmLinkTextureAnim.mouth",
        action_group="Texture Animations",
    )
    ...
    for frame in range(frameCount):
        ...
        translation = getTranslationRelativeToRest(
            boneList[0], mathutils.Vector([mmTranslationValue(currentFrame[i], actorScale) for i in range(3)])
        )
...
def mmTranslationValue(value, actorScale):
