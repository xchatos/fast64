...
from .data import Mm_ObjectData
from .cutscene.motion.constants import mmEnumCSMotionCamMode
from .mm_constants import mmData


def upgradeObjectList(objList: CollectionProperty, objData: Mm_ObjectData):
    ...
    for obj in objList:
        ...
        if "objectID" in obj:
            # ``obj["objectID"]`` returns the index inside ``mmEnumObjectIDLegacy``
            # since Blender saves the index of the element of the EnumProperty
            objectID = objData.mmEnumObjectIDLegacy[obj["objectID"]][0]
            if objectID == "Custom":
...
def upgradeRoomHeaders(roomObj: Object, objData: Mm_ObjectData):
    """Main upgrade logic for room headers"""
    altHeaders = roomObj.mmAlternateRoomHeaders
    for sceneLayer in [
        roomObj.mmRoomHeader,
...
def upgradeCutsceneMotion(csMotionObj: Object):
    ...
    if csMotionObj.type == "EMPTY":
        csMotionProp = csMotionObj.mmCSMotionProperty
        ...
        if "zc_alist" in csMotionObj and ("Preview." in objName or "ActionList." in objName):
            ...
            csMotionObj.mmEmptyType = f"CS {'Player' if 'Link' in objName else 'Actor'} Cue {emptyTypeSuffix}"
        ...
        if "zc_apoint" in csMotionObj and "Point." in objName:
            ...
            csMotionObj.mmEmptyType = f"CS {'Player' if isPlayer else 'Actor'} Cue"
    ...
    if csMotionObj.type == "ARMATURE":
        camShotProp = csMotionObj.data.mmCamShotProp
        ...
        if "cam_mode" in csMotionObj.data:
            camShotProp.shotCamMode = mmEnumCSMotionCamMode[csMotionObj.data["cam_mode"]][0]
            del csMotionObj.data["cam_mode"]

        for bone in csMotionObj.data.bones:
            camShotPointProp = bone.mmCamShotPointProp
