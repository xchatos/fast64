...
from .room.properties import MMRoomHeaderProperty, MMAlternateRoomHeaderProperty
from .mm_constants import mmData
from .cutscene.exporter import convertCutsceneObject, readCutsceneData
from .mm_spline import assertCurveValid, mmConvertPath
from .mm_model_classes import MMModel
from .mm_collision import MMCameraData, exportCollisionCommon
from .mm_collision_classes import MMCameraPosData, MMWaterBox, MMCrawlspaceData, decomp_compat_map_CameraSType
from .mm_object import addMissingObjectsToAllRoomHeaders
from .mm_f3d_writer import writeTextureArraysNew, writeTextureArraysExisting1D
...
from .mm_utility import (
    ...
    mmGetPath,
    getCustomProperty,
    mmConvertTranslation,
    mmConvertRotation,
    getSceneDirFromLevelName,
    isPathObject,
)

from .mm_level_classes import (
    MMLight,
    MMExit,
    MMScene,
    MMRoom,
...
def mmPreprendSceneIncludes(scene, file):
    exportFile = getIncludes(scene)
    exportFile.append(file)
    return exportFile


def mmCreateSceneHeader(levelC):
    sceneHeader = CData()
...
def mmCombineSceneFiles(levelC):
    sceneC = CData()
...
def mmExportSceneToC(originalSceneObj, transformMatrix, sceneName, DLFormat, savePNG, exportInfo, bootToSceneOptions):
    checkObjectReference(originalSceneObj, "Scene object")
    isCustomExport = exportInfo.isCustomExportPath
    exportPath = exportInfo.exportPath

    scene = mmConvertScene(originalSceneObj, transformMatrix, sceneName, DLFormat, not savePNG)
    ...
    roomObjList = [
        obj for obj in originalSceneObj.children_recursive if obj.data is None and obj.mmEmptyType == "Room"
    ]
    for roomObj in roomObjList:
        room = scene.rooms[roomObj.mmRoomHeader.roomIndex]
        addMissingObjectsToAllRoomHeaders(roomObj, room, mmData)

    sceneInclude = exportSubdir + "/" + sceneName + "/"
    levelPath = mmGetPath(exportPath, isCustomExport, exportSubdir, sceneName, True, True)
    ...
    if bpy.context.scene.mmSceneExportSettings.singleFile:
        writeCDataSourceOnly(
            mmPreprendSceneIncludes(scene, mmCombineSceneFiles(levelC)),
        ...
        for i in range(len(scene.rooms)):
            ...
            writeCDataSourceOnly(
                mmPreprendSceneIncludes(scene, roomC), os.path.join(levelPath, scene.rooms[i].roomName() + ".c")
            )
    else:
        # Export the scene segment .c files
        writeCDataSourceOnly(
            mmPreprendSceneIncludes(scene, levelC.sceneMainC), os.path.join(levelPath, scene.sceneName() + "_main.c")
        )
        if levelC.sceneTexturesIsUsed():
            writeCDataSourceOnly(
                mmPreprendSceneIncludes(scene, levelC.sceneTexturesC),
                os.path.join(levelPath, scene.sceneName() + "_tex.c"),
            )
        writeCDataSourceOnly(
            mmPreprendSceneIncludes(scene, levelC.sceneCollisionC),
            os.path.join(levelPath, scene.sceneName() + "_col.c"),
        )
        if levelC.sceneCutscenesIsUsed():
            for i in range(len(levelC.sceneCutscenesC)):
                writeCDataSourceOnly(
                    mmPreprendSceneIncludes(scene, levelC.sceneCutscenesC[i]),
        ...
        for roomName, roomMainC in levelC.roomMainC.items():
            writeCDataSourceOnly(
                mmPreprendSceneIncludes(scene, roomMainC), os.path.join(levelPath, roomName + "_main.c")
            )
        for roomName, roomShapeInfoC in levelC.roomShapeInfoC.items():
            writeCDataSourceOnly(
                mmPreprendSceneIncludes(scene, roomShapeInfoC), os.path.join(levelPath, roomName + "_model_info.c")
            )
        for roomName, roomModelC in levelC.roomModelC.items():
            writeCDataSourceOnly(
                mmPreprendSceneIncludes(scene, roomModelC), os.path.join(levelPath, roomName + "_model.c")
            )

    # Export the scene .h file
    writeCDataHeaderOnly(mmCreateSceneHeader(levelC), os.path.join(levelPath, scene.sceneName() + ".h"))
...
def writeTextureArraysExistingScene(fModel: MMModel, exportPath: str, sceneInclude: str):
...
def readSceneData(scene, scene_properties, sceneHeader, alternateSceneHeaders):
    ...
    if scene.writeCutscene:
        ...
        if scene.csWriteType == "Embedded":
            ...
            elif sceneHeader.csWriteObject.mmEmptyType != "Cutscene":
    ...
    if alternateSceneHeaders is not None:
        ...
        scene.collision.cameraData = MMCameraData(scene.name)
...
def getConvertedTransformWithOrientation(transformMatrix, sceneObj, obj, orientation):
    ...
    convertedTranslation = mmConvertTranslation(blenderTranslation)
    convertedRotation = mmConvertRotation(rotation)
...
def getExitData(exitProp):
    ...
    return MMExit(exitProp.exitIndexCustom)


def getLightData(lightProp):
    light = MMLight()
    light.ambient = exportColor(lightProp.ambient)
    light.diffuse0, light.diffuseDir0 = mmGetBaseOrCustomLight(lightProp, 0, True, True)
    light.diffuse1, light.diffuseDir1 = mmGetBaseOrCustomLight(lightProp, 1, True, True)
...
def readRoomData(
    sceneName: str,
    room: MMRoom,
    roomHeader: MMRoomHeaderProperty,
    alternateRoomHeaders: MMAlternateRoomHeaderProperty,
):
    ...
    for obj in roomHeader.objectList:
        ...
        if "objectID" not in obj:
            if obj.objectKey != "Custom":
                objectID = mmData.objectData.objectsByKey[obj.objectKey].id
            else:
                objectID = obj.objectIDCustom
        else:
            objectID = mmData.objectData.mmEnumObjectIDLegacy[obj["objectID"]][0]
            if objectID == "Custom":
                objectID = obj.objectIDCustom
...
def readCamPos(camPosProp, obj, scene, sceneObj, transformMatrix):
    ...
    camPosProp = obj.mmCameraPositionProperty
    ...
    scene.collision.cameraData.camPosDict[index] = MMCameraPosData(
        camSType,
...
def readCrawlspace(obj, scene, transformMatrix):
    splineProp = obj.mmSplineProperty
    ...
    crawlspace = MMCrawlspaceData(camSType)
...
def readPathProp(pathProp, obj, scene, sceneObj, sceneName, transformMatrix):
    relativeTransform = transformMatrix @ sceneObj.matrix_world.inverted() @ obj.matrix_world
    # scene.pathList[obj.name] = mmConvertPath(sceneName, obj, relativeTransform)

    # actorProp should be an actor, but its purpose is to access headerSettings so this also works.
    addActor(scene, mmConvertPath(sceneName, obj, relativeTransform), obj.mmSplineProperty, "pathList", obj.name)


def mmConvertScene(originalSceneObj, transformMatrix, sceneName, DLFormat, convertTextureData):
    if originalSceneObj.data is not None or originalSceneObj.mmEmptyType != "Scene":
    ...
    sceneObj, allObjs = mmDuplicateHierarchy(originalSceneObj, None, True, MMObjectCategorizer())

    if bpy.context.scene.exportHiddenGeometry:
        restoreHiddenState(hiddenState)

    roomObjs = [child for child in sceneObj.children_recursive if child.data is None and child.mmEmptyType == "Room"]
    if len(roomObjs) == 0:
        raise PluginError("The scene has no child empties with the 'Room' empty type.")

    try:
        scene = MMScene(sceneName, MMModel(sceneName + "_dl", DLFormat, None))
        readSceneData(scene, sceneObj.fast64.mm.scene, sceneObj.mmSceneHeader, sceneObj.mmAlternateSceneHeaders)
        processedRooms = set()
        ...
        for obj in sceneObj.children_recursive:
            ...
            if obj.data is None and obj.mmEmptyType == "Room":
                roomObj = obj
                ...
                readRoomData(sceneName, room, roomHeader, roomObj.mmAlternateRoomHeaders)
                ...
                cullGroup = CullGroup(translation, scale, obj.mmRoomHeader.defaultCullDistance)
                ...
                mmProcessEmpties(scene, room, sceneObj, roomObj, transformMatrix)
            elif obj.data is None and obj.mmEmptyType == "Water Box":
                # 0x3F = -1 in 6bit value
                mmProcessWaterBox(sceneObj, obj, transformMatrix, scene, 0x3F)
                ...
                if isPathObject(obj):
                    readPathProp(obj.mmSplineProperty, obj, scene, sceneObj, sceneName, transformMatrix)
                else:
        ...
        mmCleanupScene(originalSceneObj, allObjs)
...
def mmProcessMesh(
    ...
    if obj.data is None and obj.mmEmptyType == "Cull Group":
        ...
        cullProp = obj.mmCullGroupProperty
        checkUniformScale(scale, obj)
        DLGroup = roomMesh.addMeshGroup(
            CullGroup(
                mmConvertTranslation(translation),
    ...
    for childObj in alphabeticalChildren:
        if childObj.data is None and childObj.mmEmptyType == "LOD":
            mmProcessLOD(
                roomMesh,
                DLGroup,
                sceneObj,
...
def mmProcessLOD(
    ...
    relativeTransform = transformMatrix @ sceneObj.matrix_world.inverted() @ obj.matrix_world
    translation, rotation, scale = relativeTransform.decompose()
    mmTranslation = mmConvertTranslation(translation)

    LODHierarchyObject = obj
    name = toAlnum(roomMesh.model.name + "_" + obj.name + "_lod")
    opaqueLOD = roomMesh.model.addLODGroup(name + "_opaque", mmTranslation, obj.f3d_lod_always_render_farthest)
    transparentLOD = roomMesh.model.addLODGroup(
        name + "_transparent", mmTranslation, obj.f3d_lod_always_render_farthest
    )

    index = 0
    for childObj in obj.children:
        # This group will not be converted to C directly, but its display lists will be converted through the FLODGroup.
        childDLGroup = MMDLGroup(name + str(index), roomMesh.model.DLFormat)
        index += 1
        ...
        opaqueLOD.add_lod(childDLGroup.opaque, childObj.f3d_lod_z * bpy.context.scene.mmBlenderScale)
        transparentLOD.add_lod(childDLGroup.transparent, childObj.f3d_lod_z * bpy.context.scene.mmBlenderScale)
...
def mmProcessEmpties(scene, room, sceneObj, obj, transformMatrix):
    translation, rotation, scale, orientedRotation = getConvertedTransform(transformMatrix, sceneObj, obj, True)

    if obj.data is None:
        if obj.mmEmptyType == "Actor":
            actorProp = obj.mmActorProperty
            ...
            if actorProp.actorID != "None":
                ...
                actorName = (
                    mmData.actorData.actorsByID[actorProp.actorID].name.replace(
            ...
            transActorProp = obj.mmTransitionActorProperty
            if transActorProp.actor.actorID != "None":
                ...
                transActorName = (
                    mmData.actorData.actorsByID[transActorProp.actor.actorID].name.replace(
            ...
            spawnIndex = obj.mmEntranceProperty.spawnIndex
            addActor(scene, MMEntrance(room.roomIndex, spawnIndex), entranceProp.actor, "entranceList", obj.name)
            addStartPosition(
                scene,
                spawnIndex,
                MMActor(
            ...
            mmProcessWaterBox(sceneObj, obj, transformMatrix, scene, room.roomIndex)
    elif isinstance(obj.data, bpy.types.Camera):
        camPosProp = obj.mmCameraPositionProperty
        readCamPos(camPosProp, obj, scene, sceneObj, transformMatrix)
    elif isinstance(obj.data, bpy.types.Curve) and assertCurveValid(obj):
        if isPathObject(obj):
            readPathProp(obj.mmSplineProperty, obj, scene, sceneObj, scene.name, transformMatrix)
        else:
            readCrawlspace(obj, scene, transformMatrix)

    for childObj in obj.children:
        mmProcessEmpties(scene, room, sceneObj, childObj, transformMatrix)


def mmProcessWaterBox(sceneObj, obj, transformMatrix, scene, roomIndex):
    translation, rotation, scale, orientedRotation = getConvertedTransform(transformMatrix, sceneObj, obj, True)

    checkIdentityRotation(obj, orientedRotation, False)
    waterBoxProp = obj.mmWaterBoxProperty
