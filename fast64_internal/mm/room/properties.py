...
from ..mm_utility import drawCollectionOps, onMenuTabChange, onHeaderMenuTabChange, drawEnumWithCustom, drawAddButton
from ..mm_upgrade import upgradeRoomHeaders
...
class MMObjectProperty(PropertyGroup):
    ...
    def upgrade_object(obj: Object):
        ...
        upgradeRoomHeaders(obj, mmData.objectData)
...
class MMBGProperty(PropertyGroup):
...
class MMRoomHeaderProperty(PropertyGroup):
    expandTab: BoolProperty(name="Expand Tab")
    menuTab: EnumProperty(items=mmEnumRoomMenu, update=onMenuTabChange)
    altMenuTab: EnumProperty(items=mmEnumRoomMenuAlternate)
    ...
    roomBehaviour: EnumProperty(items=mmEnumRoomBehaviour, default="0x00")
    ...
    linkIdleMode: EnumProperty(name="Link Idle Mode", items=mmEnumLinkIdle, default="0x00")
    ...
    objectList: CollectionProperty(type=MMObjectProperty)

    roomShape: EnumProperty(items=mmEnumRoomShapeType, default="ROOM_SHAPE_TYPE_NORMAL")
    defaultCullDistance: IntProperty(name="Default Cull Distance", min=1, default=100)
    bgImageList: CollectionProperty(type=MMBGProperty)
    ...
    def draw_props(self, layout: UILayout, dropdownLabel: str, headerIndex: int, objName: str):
        from ..props_panel_main import MM_ManualUpgrade
        ...
        if menuTab == "General":
            ...
            if isLegacy:
                ...
                upgradeLayout.operator(MM_ManualUpgrade.bl_idname, text="Upgrade Data Now!")
            objBox.enabled = False if isLegacy else True

            drawAddButton(objBox, len(self.objectList), "Object", headerIndex, objName)


class MMAlternateRoomHeaderProperty(PropertyGroup):
    childNightHeader: PointerProperty(name="Child Night Header", type=MMRoomHeaderProperty)
    adultDayHeader: PointerProperty(name="Adult Day Header", type=MMRoomHeaderProperty)
    adultNightHeader: PointerProperty(name="Adult Night Header", type=MMRoomHeaderProperty)
    cutsceneHeaders: CollectionProperty(type=MMRoomHeaderProperty)
...
classes = (
    MMObjectProperty,
    MMBGProperty,
    MMRoomHeaderProperty,
    MMAlternateRoomHeaderProperty,
)
...
def mm_room_props_register():
    ...
    Object.mmRoomHeader = PointerProperty(type=MMRoomHeaderProperty)
    Object.mmAlternateRoomHeaders = PointerProperty(type=MMAlternateRoomHeaderProperty)


def mm_room_props_unregister():
    del bpy.types.Object.mmRoomHeader
    del bpy.types.Object.mmAlternateRoomHeaders
