from bpy.types import Panel, Curve
from bpy.utils import register_class, unregister_class
from ...utility import prop_split
from ..mm_utility import getSceneObj, drawEnumWithCustom
from ..actor.properties import MMActorHeaderProperty
from .properties import MMSplineProperty


class MMSplinePanel(Panel):
    bl_label = "Spline Inspector"
    bl_idname = "OBJECT_PT_MM_Spline_Inspector"
    bl_space_type = "PROPERTIES"
    bl_region_type = "WINDOW"
    bl_context = "object"
    bl_options = {"HIDE_HEADER"}

    @classmethod
    def poll(cls, context):
        return context.scene.gameEditorMode == "MM" and (
            context.object is not None and type(context.object.data) == Curve
        )

    def draw(self, context):
        box = self.layout.box().column()
        box.box().label(text="MM Spline Inspector")
        curve = context.object.data
        if curve.splines[0].type != "NURBS":
            box.label(text="Only NURBS curves are compatible.")
        else:
            sceneObj = getSceneObj(context.object)
            altSceneProp = sceneObj.mmAlternateSceneHeaders if sceneObj is not None else None
            splineProp: MMSplineProperty = context.object.mmSplineProperty
            splineProp.draw_props(box, altSceneProp, context.object.name)


mm_spline_panel_classes = (MMSplinePanel,)


def mm_spline_panels_register():
    for cls in mm_spline_panel_classes:
        register_class(cls)


def mm_spline_panels_unregister():
    for cls in mm_spline_panel_classes:
        unregister_class(cls)
