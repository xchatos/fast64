...
from .io_classes import MMCSMotionObjectFactory
from .constants import mmEnumCSActorCueListCommandType
...
def getActorCueList(operator: Operator, context: Context) -> Object | None:
    ...
    if activeObj.mmEmptyType == "CS Actor Cue" and activeObj.parent.mmEmptyType == "CS Actor Cue List":
        cueListObj = activeObj.parent

    if not cueListObj.mmEmptyType == "CS Actor Cue List" and cueListObj.parent.mmEmptyType == "Cutscene":
...
def createNewActorCueList(csObj: Object, isPlayer: bool):
    ...
    objFactory = MMCSMotionObjectFactory()
...
class MMCSMotionPlayPreview(Operator):
    ...
    def execute(self, context):
        try:
            ...
            if csObj is not None:
                ...
                bpy.context.scene.mmCSPreviewCSObj = csObj
                bpy.context.scene.mmCSPreviewNodesReady = False
                setupCompositorNodes()
                initFirstFrame(csObj, bpy.context.scene.mmCSPreviewNodesReady, cameraObj)
...
class MMCSMotionAddBone(Operator):
...
class MMCSMotionAddActorCue(Operator):
    ...
    def execute(self, context):
        ...
        try:
            if actorCueListObj is not None:
                if actorCueListObj.parent is not None and actorCueListObj.parent.mmEmptyType != "Cutscene":
                ...
                if len(actorCueListObj.children) > 0:
                    ...
                    for i, obj in enumerate(actorCueListObj.children):
                        if obj.mmEmptyType == "CS Dummy Cue":
                            dummyCue = obj
                    ...
                    if dummyCue is not None:
                        ...
                        startFrame = dummyCue.mmCSMotionProperty.actorCueProp.cueStartFrame
                        ...
                        dummyCue.mmCSMotionProperty.actorCueProp.cueStartFrame += 1
                ...
                getEndFrame = newActorCueObj.mmCSMotionProperty.actorCueProp.cueEndFrame
...
class MMCSMotionCreateActorCuePreview(Operator):
    ...
    def execute(self, context):
        ...
        if cueList is not None and cueList.mmEmptyType in ["CS Actor Cue List", "CS Player Cue List"]:
            isCueMoving = setupActorCuePreview(
                cueList.parent, "Actor" if "Actor" in cueList.mmEmptyType else "Player", True, cueList
            )
...
class MMCSMotionCreateCameraShot(Operator):
...
class MMCSMotionCreatePlayerCueList(Operator):
...
class MMCSMotionCreateActorCueList(Operator):
...
class MM_SearchActorCueCmdTypeEnumOperator(Operator):
    bl_idname = "object.mm_search_actorcue_cmdtype_enum_operator"
    bl_label = "Select Command Type"
    bl_property = "commandType"
    bl_options = {"REGISTER", "UNDO"}

    commandType: EnumProperty(items=mmEnumCSActorCueListCommandType, default="actor_cue_0_0")
    objName: StringProperty()

    def execute(self, context):
        obj = bpy.data.objects[self.objName]
        obj.mmCSMotionProperty.actorCueListProp.commandType = self.commandType
...
class MM_SearchPlayerCueIdEnumOperator(Operator):
    bl_idname = "object.mm_search_playercueid_enum_operator"
    ...
    def execute(self, context):
        obj = bpy.data.objects[self.objName]
        obj.mmCSMotionProperty.actorCueProp.playerCueID = self.playerCueID
...
def mm_csMotion_ops_register():
    for cls in classes:
        register_class(cls)


def mm_csMotion_ops_unregister():
