...
def getCmdCamState(shotObj: Object, frame: int):
    frame -= shotObj.data.mmCamShotProp.shotStartFrame + 1
...
def getCutsceneCamState(csObj: Object, frame: int):
    ...
    if len(shotObjects) > 0:
        ...
        for obj in shotObjects:
            if obj.data.mmCamShotProp.shotStartFrame < frame and obj.data.mmCamShotProp.shotStartFrame > startFrame:
                shotObj = obj
                startFrame = obj.data.mmCamShotProp.shotStartFrame
...
def getActorCueState(cueListObj: Object, frame: int):
    ...
    for cueObj in cueListObj.children:
        ...
        if obj is not None:
            cueList.append(obj)
    cueList.sort(key=lambda o: o.mmCSMotionProperty.actorCueProp.cueStartFrame)

    if len(cueList) >= 2:
        for i in range(len(cueList) - 1):
            startFrame = cueList[i].mmCSMotionProperty.actorCueProp.cueStartFrame
            endFrame = cueList[i + 1].mmCSMotionProperty.actorCueProp.cueStartFrame
...
def previewFrameHandler(scene: Scene):
    for obj in bpy.data.objects:
        ...
        if (
            ...
            and parentObj.mmEmptyType == "Cutscene"
        ):
            if obj.type == "CAMERA":
                pos, rot_quat, viewAngle = getCutsceneCamState(parentObj, scene.frame_current)

                if parentObj.mmCutsceneProperty.preview.useWidescreen:
                    viewAngle *= 4 / 3
            ...
            elif obj.mmEmptyType in ["CS Actor Cue Preview", "CS Player Cue Preview"]:
                cueListToPreview = None
                if "Actor" in obj.mmEmptyType:
                    cueListToPreview = obj.mmCSMotionProperty.actorCueListProp.actorCueListToPreview
                elif "Player" in obj.mmEmptyType:
                    cueListToPreview = obj.mmCSMotionProperty.actorCueListProp.playerCueListToPreview
                else:
...
def mm_csMotion_preview_register():
    bpy.app.handlers.frame_change_pre.append(previewFrameHandler)


def mm_csMotion_preview_unregister():
