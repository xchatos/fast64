...
from ..constants import (
    mmCSMotionLegacyToNewCmdNames,
    mmCSMotionListCommands,
    mmCSMotionCSCommands,
    mmCSMotionListEntryCommands,
    mmCSMotionSingleCommands,
    mmCSMotionListAndSingleCommands,
)
...
from ..io_classes import (
    ...
    MMCSMotionCamPoint,
    MMCSMotionCutscene,
    MMCSMotionObjectFactory,
    MMCSMotionMisc,
    MMCSMotionMiscList,
    MMCSMotionTransition,
)
...
class MMCSMotionImportCommands:
    ...
    def getNewCutscene(self, csData: str, name: str):
        params = self.getCmdParams(csData, "CS_BEGIN_CUTSCENE", MMCSMotionCutscene.paramNumber)
        return MMCSMotionCutscene(name, self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewActorCueList(self, cmdData: str, isPlayer: bool):
        paramNumber = MMCSMotionActorCueList.paramNumber
        ...
        if isPlayer:
            actorCueList = MMCSMotionActorCueList("Player", params[0])
        else:
            ...
            actorCueList = MMCSMotionActorCueList(commandType, self.getInteger(params[1].strip()))

        return actorCueList

    def getNewCamEyeSpline(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_CAM_EYE_SPLINE", MMCSMotionCamEyeSpline.paramNumber)
        return MMCSMotionCamEyeSpline(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewCamATSpline(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_CAM_AT_SPLINE", MMCSMotionCamATSpline.paramNumber)
        return MMCSMotionCamATSpline(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewCamEyeSplineRelToPlayer(self, cmdData: str):
        params = self.getCmdParams(
            cmdData, "CS_CAM_EYE_SPLINE_REL_TO_PLAYER", MMCSMotionCamEyeSplineRelToPlayer.paramNumber
        )
        return MMCSMotionCamEyeSplineRelToPlayer(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewCamATSplineRelToPlayer(self, cmdData: str):
        params = self.getCmdParams(
            cmdData, "CS_CAM_AT_SPLINE_REL_TO_PLAYER", MMCSMotionCamATSplineRelToPlayer.paramNumber
        )
        return MMCSMotionCamATSplineRelToPlayer(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewCamEye(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_CAM_EYE", MMCSMotionCamEye.paramNumber)
        return MMCSMotionCamEye(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewCamAT(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_CAM_AT", MMCSMotionCamAT.paramNumber)
        return MMCSMotionCamAT(self.getInteger(params[0]), self.getInteger(params[1]))

    def getNewActorCue(self, cmdData: str, isPlayer: bool):
        params = self.getCmdParams(
            cmdData, f"CS_{'PLAYER' if isPlayer else 'ACTOR'}_CUE", MMCSMotionActorCue.paramNumber
        )

        return MMCSMotionActorCue(
    ...
    def getNewCamPoint(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_CAM_POINT", MMCSMotionCamPoint.paramNumber)

        return MMCSMotionCamPoint(
            params[0],
            self.getInteger(params[1]),
            self.getInteger(params[2]),
            float(params[3][:-1]),
    ...
    def getNewMisc(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_MISC", MMCSMotionMisc.paramNumber)
        miscEnum = mmData.enumData.enumByKey["csMiscType"]
        item = miscEnum.itemById.get(params[0])
        if item is None:
            item = miscEnum.itemByIndex[self.getInteger(params[0])]
        miscType = item.id
        return MMCSMotionMisc(self.getInteger(params[1]), self.getInteger(params[2]), miscType)

    def getNewMiscList(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_MISC_LIST", MMCSMotionMiscList.paramNumber)
        return MMCSMotionMiscList(params[0])

    def getNewTransition(self, cmdData: str):
        params = self.getCmdParams(cmdData, "CS_TRANSITION", MMCSMotionTransition.paramNumber)
        transitionEnum = mmData.enumData.enumByKey["csTransitionType"]
        item = transitionEnum.itemById.get(params[0])
        if item is None:
            item = transitionEnum.itemByIndex[self.getInteger(params[0])]
        transType = item.id
        return MMCSMotionTransition(self.getInteger(params[1]), self.getInteger(params[2]), transType)


@dataclass
class MMCSMotionImport(MMCSMotionImportCommands, MMCSMotionObjectFactory):
    ...
    def getParsedCutscenes(self):
        ...
        # replace old names
        oldNames = list(mmCSMotionLegacyToNewCmdNames.keys())
        fileData = fileData.replace("CS_CMD_CONTINUE", "CS_CAM_CONTINUE")
        fileData = fileData.replace("CS_CMD_STOP", "CS_CAM_STOP")
        for oldName in oldNames:
            fileData = fileData.replace(f"{oldName}(", f"{mmCSMotionLegacyToNewCmdNames[oldName]}(")
        ...
        for cutscene in cutsceneList:
            ...
            for line in cutscene:
                ...
                if csName is not None and not "CS_UNK_DATA" in curCmd:
                    if curCmd in mmCSMotionCSCommands:
                        line = line.removesuffix(",") + "\n"

                        if curCmd in mmCSMotionSingleCommands and curCmd != "CS_END":
                            parsedData += line

                        if not cmdListFound and curCmd in mmCSMotionListCommands:
                            cmdListFound = True
                            parsedData = ""
                        ...
                        if curCmdPrefix is not None:
                            if curCmdPrefix in curCmd:
                                parsedData += line
                            elif not cmdListFound and curCmd in mmCSMotionListEntryCommands:
                                print(f"{csName}, command:\n{line}")
                                raise PluginError(f"ERROR: Found a list entry outside a list inside ``{csName}``!")

                        if cmdListFound and nextCmd == "CS_END" or nextCmd in mmCSMotionListAndSingleCommands:
                            cmdListFound = False
    ...
    def getCutsceneList(self):
        ...
        cutsceneList: list[MMCSMotionCutscene] = []
        cmdDataList = [
    ...
    def setActorCueData(self, csObj: Object, actorCueList: list[MMCSMotionActorCueList], cueName: str, csNbr: int):
        ...
        for obj, endFrame in zip(cueObjList, cueEndFrames):
            ...
            getEndFrame = obj.mmCSMotionProperty.actorCueProp.cueEndFrame

            if endFrame != getEndFrame and obj.mmEmptyType != "CS Dummy Cue":
                print(f"WARNING: `{obj.name}`'s end frame do not match the one from the script!")

    def validateCameraData(self, cutscene: MMCSMotionCutscene):
    ...
    def setBoneData(
        self, cameraShotObj: Object, boneData: list[tuple[MMCSMotionCamPoint, MMCSMotionCamPoint]], csNbr: int
    ):
        ...
        for i, (eyePoint, atPoint) in enumerate(boneData, 1):
            ...
            newBone.mmCamShotPointProp.shotPointFrame = atPoint.frame
            newBone.mmCamShotPointProp.shotPointViewAngle = atPoint.viewAngle
            newBone.mmCamShotPointProp.shotPointRoll = atPoint.camRoll
    ...
    def setCameraShotData(
        ...
        for i, (camEyeSpline, camATSpline) in enumerate(zip(eyePoints, atPoints), startIndex):
            ...
            cameraShotObj.data.mmCamShotProp.shotStartFrame = camEyeSpline.startFrame
            cameraShotObj.data.mmCamShotProp.shotCamMode = camMode
        ...
        for i, cutscene in enumerate(cutsceneList, csNumber):
            ...
            if len(cutscene.miscList) > 0:
                for miscList in cutscene.miscList:
                    for miscCmd in miscList.entries:
                        miscProp = csObj.mmCutsceneProperty.preview.miscList.add()
            ...
            if len(cutscene.transitionList) > 0:
                for transition in cutscene.transitionList:
                    transitionProp = csObj.mmCutsceneProperty.preview.transitionList.add()
