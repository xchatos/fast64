...
from ...mm_utility import mmParseRotation
...
def createNewBone(cameraShotObj: Object, name: str, headPos: list[float], tailPos: list[float]):
    ...
    newBone.mmCamShotPointProp.shotPointFrame = 30
    newBone.mmCamShotPointProp.shotPointViewAngle = 60.0
    newBone.mmCamShotPointProp.shotPointRoll = 0


def createNewCameraShot(csObj: Object):
    from .io_classes import MMCSMotionObjectFactory  # circular import fix

    index, csPrefix = getNameInformations(csObj, "Camera Shot", None)

    # create a basic armature
    name = f"{csPrefix}.Camera Shot {index:02}"
    newCameraShotObj = MMCSMotionObjectFactory().getNewArmatureObject(name, True, csObj)
...
def getBlenderPosition(pos: list[int], scale: int):
    """Returns the converted Mm position"""

    # Mm: +X right, +Y up, -Z forward
...
def getBlenderRotation(rotation: list[str]):
    """Returns the converted Mm rotation"""

    rot = [int(getRotation(r), base=16) for r in rotation]
    return yUpToZUp @ Vector(mmParseRotation(rot))
...
def getCSMotionValidateObj(csObj: Object, obj: Object, target: str):
    ...
    if target is None or target != "Camera Shot":
        ...
        if obj.type == "EMPTY" and obj.mmEmptyType in nonListTypes or obj.mmEmptyType in listTypes:
            return obj
...
def metersToBlend(context: Context, value: float):
    return value * 56.0 / context.scene.mmBlenderScale
...
def setupActorCuePreview(csObj: Object, actorOrPlayer: str, selectObject: bool, cueList: Object):
    ...
    if isCueMoving:
        ...
        for obj in csObj.children:
            ...
            previewObj = MMCSMotionObjectFactory().getNewActorCuePreviewObject(name, selectObject, csObj)
        ...
        setattr(previewObj.mmCSMotionProperty.actorCueListProp, f"{actorOrPlayer.lower()}CueListToPreview", cueList)
...
def getCutsceneEndFrame(csObj: Object):
    ...
    for shotObj in shotObjects:
        ...
        endFrame = shotObj.data.mmCamShotProp.shotStartFrame + max(2, sum(b.frame for b in boneDataList)) + 2
...
def setupCutscene(csObj: Object):
    ...
    for obj in csObj.children:
        if obj.mmEmptyType in ["CS Actor Cue List", "CS Player Cue List"]:
            setupActorCuePreview(csObj, "Actor" if "Actor" in obj.mmEmptyType else "Player", False, obj)
