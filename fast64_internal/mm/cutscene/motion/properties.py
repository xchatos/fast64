...
from ...mm_upgrade import upgradeCutsceneMotion
from ...mm_utility import getEnumName
from ...mm_constants import mmData
from .constants import mmEnumCSMotionCamMode, mmEnumCSActorCueListCommandType

from .operators import (
    MMCSMotionAddActorCue,
    MMCSMotionCreateActorCuePreview,
    MM_SearchActorCueCmdTypeEnumOperator,
    MMCSMotionAddBone,
    MM_SearchPlayerCueIdEnumOperator,
)
...
def getNextCuesStartFrame(self):
    ...
    if parentObj is not None and parentObj.mmEmptyType in ["CS Actor Cue List", "CS Player Cue List"]:
        if curCueObj.mmEmptyType in ["CS Actor Cue", "CS Player Cue"]:
            actorCueObjList = parentObj.children
            for i, obj in enumerate(actorCueObjList):
                if obj == curCueObj:
                    return actorCueObjList[i + 1].mmCSMotionProperty.actorCueProp.cueStartFrame
    return -1


class MMCSMotionActorCueListProperty(PropertyGroup):
    commandType: EnumProperty(
        items=mmEnumCSActorCueListCommandType, name="CS Actor Cue Command Type", default="actor_cue_0_0"
    )
    ...
    def isActorCueListObj(self, obj: Object, emptyType: str):
        return obj.type == "EMPTY" and obj.mmEmptyType == emptyType
    ...
    def draw_props(self, layout: UILayout, isPreview: bool, labelPrefix: str, objName: str):
        ...
        if not isPlayer and not isPreview:
            ...
            searchOp = searchBox.operator(
                MM_SearchActorCueCmdTypeEnumOperator.bl_idname, icon="VIEWZOOM", text="Command Type:"
            )
            searchOp.objName = objName
            searchBox.label(text=getEnumName(mmEnumCSActorCueListCommandType, self.commandType))
        ...
        if not isPreview:
            split = box.split(factor=0.5)
            addCueOp = split.operator(MMCSMotionAddActorCue.bl_idname, text=f"Add {labelPrefix} Cue")
            addCueOp.isPlayer = isPlayer
            split.operator(MMCSMotionCreateActorCuePreview.bl_idname)
        else:
            split = box.split(factor=0.5)
...
class MMCSMotionActorCueProperty(PropertyGroup):
    ...
    playerCueID: EnumProperty(items=mmData.enumData.mmEnumCsPlayerCueId, default="cueid_none")
    ...
    def draw_props(self, layout: UILayout, labelPrefix: str, isDummy: bool, objName: str):
        ...
        if not isDummy:
            ...
            if isPlayer:
                split = box.split(factor=0.5)
                searchOp = split.operator(MM_SearchPlayerCueIdEnumOperator.bl_idname, icon="VIEWZOOM", text=label)
                searchOp.objName = objName
                split.label(text=getEnumName(mmData.enumData.mmEnumCsPlayerCueId, self.playerCueID))
            ...
            addCueOp = box.operator(MMCSMotionAddActorCue.bl_idname, text=f"Add {labelPrefix} Cue")
            addCueOp.isPlayer = isPlayer


class MMCSMotionCameraShotProperty(PropertyGroup):
    ...
    def getEndFrame(self, camShotObj: Object = None):
        ...
        if camShotObj.type == "ARMATURE":
            boneFrameList: list[int] = [bone.mmCamShotPointProp.shotPointFrame for bone in camShotObj.data.bones]
            # "fake" eye end frame
    ...
    def draw_props(self, layout: UILayout, label: str):
        ...
        box.operator(MMCSMotionAddBone.bl_idname)


class MMCSMotionCameraShotPointProperty(PropertyGroup):
    ...
    def setValue(self, value, propName: str):
        ...
        if activeObj.mode == "EDIT":
            ...
            if bone is not None:
                setattr(bone.mmCamShotPointProp, propName, value)
...
class MMCutsceneMotionProperty(PropertyGroup):
    actorCueListProp: PointerProperty(type=MMCSMotionActorCueListProperty)
    actorCueProp: PointerProperty(type=MMCSMotionActorCueProperty)
...
classes = (
    ...
    MMCSMotionCameraShotProperty,
    MMCSMotionCameraShotPointProperty,
    MMCutsceneMotionProperty,
)


def mm_csMotion_props_register():
    for cls in classes:
        register_class(cls)

    Object.mmCSMotionProperty = PointerProperty(type=MMCutsceneMotionProperty)
