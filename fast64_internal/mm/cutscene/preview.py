...
def setupCompositorNodes():
    ...
    if bpy.context.scene.mmCSPreviewNodesReady:
        return
    ...
    bpy.context.scene.mmCSPreviewNodesReady = True
...
def initFirstFrame(csObj: Object, useNodeFeatures: bool, defaultCam: Object):
    ...
    if useNodeFeatures:
        ...
        csObj.mmCutsceneProperty.preview.trigger = False
    csObj.mmCutsceneProperty.preview.isFixedCamSet = False
...
def processCurrentFrame(csObj: Object, curFrame: float, useNodeFeatures: bool, cameraObjects: Object):
    ...
    if useNodeFeatures:
        previewProp = csObj.mmCutsceneProperty.preview
        for transitionCmd in csObj.mmCutsceneProperty.preview.transitionList:
    ...
    for miscCmd in csObj.mmCutsceneProperty.preview.miscList:
        ...
        if curFrame == startFrame:
            if miscCmd.type == "CS_MISC_SET_LOCKED_VIEWPOINT" and not None in cameraObjects:
                bpy.context.scene.camera = cameraObjects[int(csObj.mmCutsceneProperty.preview.isFixedCamSet)]
                csObj.mmCutsceneProperty.preview.isFixedCamSet ^= True
...
def cutscenePreviewFrameHandler(scene: Scene):
    ...
    csObj: Object = bpy.context.scene.mmCSPreviewCSObj

    if csObj is None or not csObj.type == "EMPTY" and not csObj.mmEmptyType == "Cutscene":
        return
    ...
    for obj in bpy.data.objects:
        if obj.type == "CAMERA" and obj.parent is not None and obj.parent.mmEmptyType in ["Scene", "Room"]:
    ...
    if isclose(curFrame, previewProp.prevFrame, abs_tol=1) and isclose(curFrame, previewProp.nextFrame, abs_tol=1):
        processCurrentFrame(csObj, curFrame, bpy.context.scene.mmCSPreviewNodesReady, cameraObjects)
    else:
        # Simulate cutscene for all frames up to present
        for i in range(bpy.context.scene.frame_current):
            processCurrentFrame(csObj, i, bpy.context.scene.mmCSPreviewNodesReady, cameraObjects)
