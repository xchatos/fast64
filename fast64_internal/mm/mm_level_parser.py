...
from .collision.properties import MMMaterialCollisionProperty
from .mm_model_classes import MMF3DContext
from .mm_f3d_writer import getColliderMat
from .scene.exporter.to_c import getDrawConfig
from .scene.properties import MMSceneHeaderProperty, MMLightProperty, MMImportSceneSettingsProperty
from .room.properties import MMRoomHeaderProperty
from .actor.properties import MMActorProperty, MMActorHeaderProperty
...
def parseScene(
    settings: MMImportSceneSettingsProperty,
    option: str,
):
    ...
    if settings.isCustomDest:
        ...
        if option == "Custom":
            ...
            subfolder = None
        importPath = bpy.path.abspath(bpy.context.scene.mmDecompPath)
    ...
    sceneFolderPath = mmGetPath(importPath, settings.isCustomDest, importSubdir, sceneName, False, True)
    ...
    f3dContext = MMF3DContext(get_F3D_GBI(), [], bpy.path.abspath(bpy.context.scene.mmDecompPath))
    f3dContext.mat().prim_color = (0.5, 0.5, 0.5, 0.5)
    f3dContext.mat().env_color = (0.5, 0.5, 0.5, 0.5)

    parseMatrices(sceneData, f3dContext, 1 / bpy.context.scene.mmBlenderScale)
    f3dContext.addMatrix("&gMtxClear", mathutils.Matrix.Scale(1 / bpy.context.scene.mmBlenderScale, 4))
    ...
    if settings.includeCutscenes:
        bpy.context.scene.mmCSNumber = importCutsceneData(None, sceneData)

    sceneObj = parseSceneCommands(sceneName, None, None, sceneCommandsName, sceneData, f3dContext, 0, sharedSceneData)
    bpy.context.scene.mmSceneExportObj = sceneObj

    if not settings.isCustomDest:
        setCustomProperty(
            sceneObj.mmSceneHeader.sceneTableEntry, "drawConfig", getDrawConfig(sceneName), mmEnumDrawConfig
        )

    if bpy.context.scene.fast64.mm.headerTabAffectsVisibility:
...
def parseSceneCommands(
    ...
    if headerIndex == 0:
        sceneHeader = sceneObj.mmSceneHeader
    elif headerIndex < 4:
        sceneHeader = getattr(sceneObj.mmAlternateSceneHeaders, headerNames[headerIndex])
        sceneHeader.usePreviousHeader = False
    else:
        cutsceneHeaders = sceneObj.mmAlternateSceneHeaders.cutsceneHeaders
    ...
    for commandMatch in re.finditer(rf"(SCENE\_CMD\_[a-zA-Z0-9\_]*)\s*\((.*?)\)\s*,", commands, flags=re.DOTALL):
        ...
        if command == "SCENE_CMD_SOUND_SETTINGS":
            setCustomProperty(sceneHeader, "audioSessionPreset", args[0], mmEnumAudioSessionPreset)
            setCustomProperty(sceneHeader, "nightSeq", args[1], mmEnumNightSeq)
            setCustomProperty(sceneHeader, "musicSeq", args[2], mmEnumMusicSeq)
        ...
        # This must be handled after rooms, so that room objs can be referenced
            ...
            setCustomProperty(sceneHeader, "cameraMode", args[0], mmEnumCameraMode)
            setCustomProperty(sceneHeader, "mapLocation", args[1], mmEnumMapLocation)
            ...
            setCustomProperty(sceneHeader, "naviCup", args[0], mmEnumNaviHints)
            setCustomProperty(sceneHeader, "globalObject", args[1], mmEnumGlobalObject)
        ...
        # This must be handled after entrance list, so that entrance list can be referenced
            ...
            setCustomProperty(sceneHeader, "skyboxID", args[0], mmEnumSkybox)
            setCustomProperty(sceneHeader, "skyboxCloudiness", args[1], mmEnumCloudiness)
            setCustomProperty(sceneHeader, "skyboxLighting", args[2], mmEnumSkyboxLighting)
...
def parseRoomList(
    ...
    for roomMatch in re.finditer(
        ...
        roomData = readFile(roomPath)
        parseMatrices(roomData, f3dContext, 1 / bpy.context.scene.mmBlenderScale)
...
def parseRoomCommands(
    ...
    if roomObj is None:
        ...
        roomObj.mmRoomHeader.roomIndex = roomIndex
        roomObj.name = roomName

    if headerIndex == 0:
        roomHeader = roomObj.mmRoomHeader
    elif headerIndex < 4:
        roomHeader = getattr(roomObj.mmAlternateRoomHeaders, headerNames[headerIndex])
        roomHeader.usePreviousHeader = False
    else:
        cutsceneHeaders = roomObj.mmAlternateRoomHeaders.cutsceneHeaders
        while len(cutsceneHeaders) < headerIndex - 3:
            cutsceneHeaders.add()
    ...
    for commandMatch in re.finditer(rf"(SCENE\_CMD\_[a-zA-Z0-9\_]*)\s*\((.*?)\)\s*,", commands, flags=re.DOTALL):
        ...
        if command == "SCENE_CMD_ALTERNATE_HEADER_LIST":
            ...
            setCustomProperty(roomHeader, "roomBehaviour", args[0], mmEnumRoomBehaviour)
            setCustomProperty(roomHeader, "linkIdleMode", args[1], mmEnumLinkIdle)
...
def parseMeshHeader(
    ...
    if "ROOM_SHAPE_TYPE_" in roomShape:
        roomShapeIndex = [value[0] for value in mmEnumRoomShapeType].index(roomShape)
    else:
        roomShapeIndex = int(roomShape)

    roomHeader.roomShape = mmEnumRoomShapeType[roomShapeIndex][0]
...
def parseBGImage(roomHeader: MMRoomHeaderProperty, params: list[str], sharedSceneData: SharedSceneData):
...
def parseBGImageList(
    roomHeader: MMRoomHeaderProperty, sceneData: str, bgListName: str, sharedSceneData: SharedSceneData
):
    ...
    for entryMatch in searchItems:
        if roomShape == 2:
            ...
            position = yUpToZUp @ mathutils.Vector(
                [
                    hexOrDecInt(entryMatch.group(value).strip()) / bpy.context.scene.mmBlenderScale
            ...
            if sharedSceneData.includeCullGroups:
                ...
                cullObj.mmEmptyType = "Cull Group"
                cullObj.name = "Cull Group"
                cullProp = cullObj.mmCullGroupProperty
                cullProp.sizeControlsCull = False
                cullProp.manualRadius = hexOrDecInt(entryMatch.group(4).strip())
                cullObj.show_name = True
                # cullObj.empty_display_size = hexOrDecInt(entryMatch.group(4).strip()) / bpy.context.scene.mmBlenderScale
        ...
        for displayList, drawLayer in [(opaqueDL, "Opaque"), (transparentDL, "Transparent")]:
            if displayList != "0" and displayList != "NULL":
                meshObj = importMeshC(
                    sceneData, displayList, bpy.context.scene.mmBlenderScale, True, True, drawLayer, f3dContext, False
                )
...
def createEmptyWithTransform(positionValues: list[float], rotationValues: list[float]) -> bpy.types.Object:
    position = (
        yUpToZUp
        @ mathutils.Matrix.Scale(1 / bpy.context.scene.mmBlenderScale, 4)
        @ mathutils.Vector([hexOrDecInt(value) for value in positionValues])
    )
    rotation = yUpToZUp @ mathutils.Vector(mmParseRotation(rotationValues))
...
def handleActorWithRotAsParam(actorProp: MMActorProperty, actorID: str, rotation: list[int]):
...
def parseTransActorList(
    ...
    for actorMatch in re.finditer(regex, transitionActorList):
        ...
        if not sharedSceneData.addHeaderIfItemExists(actorHash, "Transition Actor", headerIndex):
            ...
            actorObj.mmEmptyType = "Transition Actor"
            actorObj.name = "Transition " + getDisplayNameFromActorID(params[4])
            transActorProp = actorObj.mmTransitionActorProperty
            ...
            setCustomProperty(transActorProp, "cameraTransitionFront", camFront, mmEnumCamTransition)
            setCustomProperty(transActorProp, "cameraTransitionBack", camBack, mmEnumCamTransition)

            actorProp = transActorProp.actor
            setCustomProperty(actorProp, "actorID", actorID, mmData.actorData.mmEnumActorID)
...
def parseEntranceList(
    sceneHeader: MMSceneHeaderProperty, roomObjs: list[bpy.types.Object], sceneData: str, entranceListName: str
):
...
def unsetAllHeadersExceptSpecified(headerSettings: MMActorHeaderProperty, headerIndex: int):
...
def parseSpawnList(
    ...
    for spawnMatch in re.finditer(regex, spawnList, flags=re.DOTALL):
        ...
        if not sharedSceneData.addHeaderIfItemExists(actorHash, "Entrance", headerIndex):
            ...
            spawnObj.mmEmptyType = "Entrance"
            spawnObj.name = "Entrance"
            spawnProp = spawnObj.mmEntranceProperty
...
def parseExitList(sceneHeader: MMSceneHeaderProperty, sceneData: str, exitListName: str):
...
def parseObjectList(roomHeader: MMRoomHeaderProperty, sceneData: str, objectListName: str):
    ...
    for object in objects:
        ...
        objByID = mmData.objectData.objectsByID.get(object)
...
def parseActorList(
    ...
    for actorMatch in re.finditer(regex, actorList, flags=re.DOTALL):
        actorHash = parseActorInfo(actorMatch, nestedBrackets) + (roomObj.mmRoomHeader.roomIndex,)
        ...
        if not sharedSceneData.addHeaderIfItemExists(actorHash, "Actor", headerIndex):
            ...
            actorObj = createEmptyWithTransform(position, [0, 0, 0] if actorID in actorsWithRotAsParam else rotation)
            actorObj.mmEmptyType = "Actor"
            actorObj.name = getDisplayNameFromActorID(actorID)
            actorProp = actorObj.mmActorProperty
...
def createCurveFromPoints(points: list[tuple[float, float, float]], name: str):
    ...
    for i in range(len(points)):
        position = yUpToZUp @ mathutils.Vector([value / bpy.context.scene.mmBlenderScale for value in points[i]])
...
def parsePath(
    ...
    splineProp = curveObj.mmSplineProperty
...
def parseLightList(
    sceneObj: bpy.types.Object,
    sceneHeader: MMSceneHeaderProperty,
    sceneData: str,
    lightListName: str,
    headerIndex: int,
):
...
def parseLight(
    lightHeader: MMLightProperty, index: int, rotation: mathutils.Euler, color: mathutils.Vector
...
def parseSurfaceParams(
    surface: tuple[int, int], polygonParams: tuple[bool, bool, bool, bool], collision: MMMaterialCollisionProperty
):
    params = surface
    ...
    setCustomProperty(collision, "floorSetting", str(getBits(params[0], 26, 4)), mmEnumFloorSetting)
    setCustomProperty(collision, "wallSetting", str(getBits(params[0], 21, 5)), mmEnumWallSetting)
    setCustomProperty(collision, "floorProperty", str(getBits(params[0], 13, 8)), mmEnumFloorProperty)
    ...
    setCustomProperty(collision, "terrain", str(getBits(params[1], 4, 2)), mmEnumCollisionTerrain)
    setCustomProperty(collision, "sound", str(getBits(params[1], 0, 4)), mmEnumCollisionSound)
...
def parseVertices(vertexList: list[str]):
    ...
    for vertexData in vertexList:
        vertex = [hexOrDecInt(value.strip()) / bpy.context.scene.mmBlenderScale for value in vertexData.split(",")]
...
def parseCamPosData(setting: str, sceneData: str, posDataName: str, index: int, objName: str, orderIndex: str):
    ...
    camProp = camObj.mmCameraPositionProperty
    setCustomProperty(camProp, "camSType", setting, mmEnumCameraSType)
    ...
    position = yUpToZUp @ mathutils.Vector(
        [hexOrDecInt(value.strip()) / bpy.context.scene.mmBlenderScale for value in posData[0].split(",")]
    )
    ...
    rotation = (
        ...
        @ mathutils.Euler(
            mmParseRotation([hexOrDecInt(value.strip()) for value in posData[1].split(",")])
        ).to_quaternion()
...
def parseCrawlSpaceData(
    ...
    curveObj.show_name = True
    crawlProp = curveObj.mmSplineProperty
    crawlProp.splineType = "Crawlspace"
    crawlProp.index = orderIndex
    setCustomProperty(crawlProp, "camSType", "CAM_SET_CRAWLSPACE", mmEnumCameraCrawlspaceSType)
...
def parseWaterBoxes(
    ...
    for waterBoxData in waterBoxList:
        ...
        topCorner = yUpToZUp @ mathutils.Vector(
            [hexOrDecInt(value) / bpy.context.scene.mmBlenderScale for value in params[0:3]]
        )
        dimensions = [hexOrDecInt(value) / bpy.context.scene.mmBlenderScale for value in params[3:5]]
        properties = hexOrDecInt(params[5])

        height = 1000 / bpy.context.scene.mmBlenderScale  # just to add volume
        ...
        waterBoxProp = waterBoxObj.mmWaterBoxProperty

        waterBoxObj.show_name = True
        waterBoxObj.mmEmptyType = "Water Box"
...
def parseDrawConfig(drawConfigName: str, sceneData: str, drawConfigData: str, f3dContext: MMF3DContext):
